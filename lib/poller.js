...
function failOnTimeout (status, timeout, uuid) {
  ...
  throw (
    `User or default timeout reached after ${t}. The analysis job is still running the server ` +
...
exports.do = async function (
  ...
  for (let i = 1; i <= maxPolls; i++) {
    ...
    if (Date.now() - start >= timeout) {
      ...
      statusResponse = await getStatus(uuid, accessToken, apiUrl)
    }
    if (statusResponse) {
  ...
  */
  /* eslint-disable no-throw-literal */
  throw (
    `Timeout reached after ${humanizeDuration(timeout)}. The analysis job is still running on the server ` +
    `and the result may become available later. UUID: ${uuid}\n`
  )
  /* eslint-enable no-throw-literal */
}
