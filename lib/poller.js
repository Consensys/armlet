  ...
  if (status < 200 || status > 299) {
    ...
    switch (status) {
      case 404:
        ...
        break
      case 500:
        msg = 'received error 500 from API server'
        break
      default:
...
/**
 * Performs the actual poll operation.
 ...
 * waiting till the analysis is finished, if necessary.
 * @param {String} uuid Analysis UUID.
 * @param {String} accessToken Auth token.
...
exports.do = async function (
  ...
  for (;;) {
    await timer(Math.min(pollStep, start + timeout - Date.now()))
    if (Date.now() - start >= timeout) failOnTimeout(timeout, uuid)
